...
struct mem_page_t* page_fault_routine(struct mem_t *mem, uint32_t addr){
	...
	int page_to_be_replaced = get_page_to_be_replaced(mem, addr);
...
int get_page_to_be_replaced(struct mem_t *mem, uint32_t addr){
	...
	int i;
	for(i=(index+1)% MEM_PAGE_COUNT; i!=index; i = (i+1)%MEM_PAGE_COUNT){
		if(mem->pages[i] != NULL)
			return i;
...
static struct mem_page_t *mem_page_create(struct mem_t *mem, uint32_t addr, int perm)
{
	...
	printf("Here %d %d\n",mem->context->pages_in_ram, mem->context->pages_allocated_in_ram);	
	uint32_t index, tag;
	struct mem_page_t *page;
	...
	if(!page  ||(mem->context->pages_in_ram == mem->context->pages_allocated_in_ram)){
		printf("COULD NOT FIND A FREE PAGE , EXECUTE PAGE FAULT ROUTINE %d %d \n", 
		 mem->context->pages_in_ram, mem->context->pages_allocated_in_ram);
		///!TODO -  PAGE FAULT ROUTINE 
	}
	else
